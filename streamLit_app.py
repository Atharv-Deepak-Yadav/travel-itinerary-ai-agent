...
with st.sidebar:
    ...
    # Inputs 
    col_flight_cities = st.columns(2)
    with col_flight_cities[0]:
        departure_city = st.text_input("🛫 Departure City", placeholder="e.g., London, Delhi, Seattle") # NEW INPUT
    with col_flight_cities[1]:
...
if not generate_btn and "itinerary" not in st.session_state:
    ...
    **How it works:**
    1. Enter your **Departure** and **Destination** cities, dates, and preferences in the sidebar on the left.
    2. Click the **Generate Itinerary** button below the inputs!
    """)
    ...
    if generate_btn:
        ...
        if not destination or not final_departure_city:
            st.error("Please enter both a **Departure City** and a **Destination** to generate the itinerary.")
            # Clear button state to prevent accidental double-run
            st.session_state.itinerary = None
        else:
            with st.spinner(f"🤖 AI Agents are researching flights from {final_departure_city} to {destination}..."):
                progress_bar = st.progress(0)
                for i in range(100):
                    time.sleep(0.01)
                    progress_bar.progress(i + 1)
                
                # Generate Data - Passes the departure city now
                itinerary_data, flight_details, total_cost, budget_symbol = generate_mock_itinerary(
                    final_departure_city, destination, start_date, duration, budget_level, currency, interests
                )
    ...
    if "itinerary" in st.session_state and st.session_state.itinerary is not None:
        data = st.session_state.itinerary
        flights = st.session_state.flights
        total_cost = st.session_state.total_cost
        budget_symbol = st.session_state.budget_symbol

        st.header(f"✈️ Your Trip from {final_departure_city} to {destination}")
        ...
        for day in data:
            # We wrap the day card in a container to enforce the card styling boundary
            with st.container():
                st.markdown(f"""
                <div class="day-card">
                ...
                # Render activities using columns for alignment
                for act in day['activities']:
