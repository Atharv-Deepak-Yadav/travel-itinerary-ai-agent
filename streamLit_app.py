...
st.markdown("""
    ...
    /* This targets the <h1> tag often generated by st.title inside the sidebar */
...
def generate_mock_itinerary(dest, start_date, duration, budget_level, currency, interests):
    ...
    flight_details = {
        "outbound": {
            ...
            "display_price": f"{budget_symbol} {single_flight_price:,}" # Format with comma
        },
        "return": {
...
def convert_itinerary_to_text_content(itinerary_data, destination, flight_data, total_cost, budget_symbol):
    ...
    # Use display_price (which includes the symbol and comma format)
    ...
    for day in itinerary_data:
        ...
        text_output += "--------------------------------------------------------\n"
        # Adjusted the format specifier for cost to align better in the text file
        for act in day['activities']:
            text_output += f"{act['time']} | {act['cost']:<12} | {act['activity']}\n"
        text_output += f"Daily Estimated Cost: {budget_symbol} {day['daily_cost']:,}\n" # Include daily cost in download
...
if not generate_btn and "itinerary" not in st.session_state:
    ...
    **How it works:**
    ...
    if generate_btn:
        if not destination:
            st.error("Please enter a **Destination** in the sidebar to generate the itinerary.")
        else:
            with st.spinner(f"🤖 AI Agents are researching {destination}..."):
                progress_bar = st.progress(0)
                # Simulating a realistic load time
                for i in range(100):
                    time.sleep(0.01)
    ...
    if "itinerary" in st.session_state:
        ...
        for day in data:
            with st.container(border=True): # Use container for better visual grouping
                st.markdown(f"""
                <div class="day-card" style="margin-bottom:0px; padding: 10px 15px;">
                    <h3 style="margin-top:0;">Day {day['day']}: {day['theme']}</h3>
                    <p style="color:#888; margin-bottom:10px;">{day['date']}</p>
                </div>
                """, unsafe_allow_html=True)
